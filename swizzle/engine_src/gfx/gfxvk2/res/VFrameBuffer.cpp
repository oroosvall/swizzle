...
namespace vk
{
    ...
    void VFrameBuffer::createColorImages()
    {
        ...
        {
            common::Resource<VkResource<VkImage>> image = mDevice->createImage(VkImageType::VK_IMAGE_TYPE_2D, mImageFormat,
                VkImageUsageFlagBits::VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VkImageUsageFlagBits::VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                { mWidth, mHeight, 1u }, 1u);

            VkMemoryRequirements memreq;
            vkGetImageMemoryRequirements(mDevice->getDeviceHandle(), image->getVkHandle(), &memreq);

            common::Resource<DeviceMemory> memory = mDevice->allocateMemory(VkMemoryPropertyFlagBits::VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, memreq);
    ...
    void VFrameBuffer::createDepthImage()
    {
        if (mHasDepthBuffer)
        {
            mDepthImage.mImage = mDevice->createImage(VkImageType::VK_IMAGE_TYPE_2D, VkFormat::VK_FORMAT_D32_SFLOAT_S8_UINT,
            ...
            mDepthImage.mImageMemory = mDevice->allocateMemory(VkMemoryPropertyFlagBits::VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, memreq);
    ...
    void VFrameBuffer::createRenderPass()
    {
        ...
        if (mHasDepthBuffer)
        {
            attachments.emplace_back(VkAttachmentReference{ counter, VkImageLayout::VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL });
