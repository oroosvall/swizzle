...
static ImColor GetNodeTypeColor(imext::NodeType type);
...
static inline void DrawNodeInput(imext::Input& in)
{
    ...
    else if (in.mType == imext::NodeType::Vec2)
    {
        ImGui::BeginGroup();
    ...
    else if (in.mType == imext::NodeType::Vec3)
    {
        ImGui::BeginGroup();
        ImGui::PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_ItemSpacing, {0, 1});
        ImGui::DragFloat("##value1", in.mInputData);
    ...
    else if (in.mType == imext::NodeType::Vec4)
    {
        ImGui::BeginGroup();
        ImGui::PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_ItemSpacing, {0, 1});
        ImGui::DragFloat("##value1", in.mInputData);
    ...
    else if (in.mType == imext::NodeType::Color)
    {
        ImGui::BeginGroup();
...
static ImColor GetNodeTypeColor(imext::NodeType type)
{
    ...
    case imext::NodeType::Float:
        c = IM_COL32(181, 181, 181, 255);
        break;
    case imext::NodeType::Vec2:
        c = IM_COL32(194, 0, 0, 255);
        break;
    case imext::NodeType::Vec3:
        c = IM_COL32(41, 62, 181, 255);
        break;
    case imext::NodeType::Vec4:
        c = IM_COL32(204, 79, 197, 255);
        break;
    case imext::NodeType::Color:
        c = IM_COL32(204, 204, 0, 255);
        break;
    default:
        break;
    }
...
namespace imext
{
    ...
    void ShaderGraph(const char* name, bool* open, ShaderGraphController* sgc)
    {
        ...
        for (size_t i = 0ull; i < nodes.size(); ++i)
        {
            ...
            std::vector<NodeSlot> nodeSlots; // contains positions of all node slots
            // LinkButtons
            // Input
            ...
            for (int slot_idx = 0; slot_idx < inputs.size(); slot_idx++)
            {
                if (inputs[slot_idx].mPluggable)
                {
                    ...
                    ImGui::PopID();
                    nodeSlots.push_back({nPos, GetNodeTypeColor(inputs[slot_idx].mType)});
                }
            ...
            for (int slot_idx = 0; slot_idx < outputs.size(); slot_idx++)
            {
                ...
                ImGui::PopID();
                nodeSlots.push_back({nPos, GetNodeTypeColor(outputs[slot_idx].mType)});
            }
            ...
            // Render Node slots
            for (auto& n : nodeSlots)
            {
                draw_list->AddCircleFilled(n.mPos, NODE_SLOT_RADIUS, n.mColor);
            }
            ImGui::PopID();
        }
